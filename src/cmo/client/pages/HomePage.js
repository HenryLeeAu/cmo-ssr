  ...
  show() {
    ...
    return (
      <div>
        ...
        <div className="row">
          <div className="col-6">
            {response.main!==undefined  && <LatestArticles {...response.main.latestArticles} />}
            {response.main!==undefined  && <Blogs {...response.main.blogs} />}
            {response.main!==undefined  && <LatestMultimedia {...response.main.latestMultimedia} />}
          </div>
          <div className="col-6">
          {response.main!==undefined  && <Trendsetting {...response.main.trendSettings} />}
          </div>
        </div>
      </div>
    )
...
*/
export default  SiteWrapperWithRHS(connect(mapStateToProps, { fetchHome, clearData })(HomePage));
