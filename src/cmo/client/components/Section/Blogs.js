    ...
    <SectionBase
      ...
      <ul >
        {props.list && props.list.map((article, index) => {
          const { subCategory, publishedDate, imageUrl, title, description, slug, author } = article
          const articleLink=`/${props.category}/${subCategory}/${publishedDate.year}/${publishedDate.month}/${publishedDate.date}/${slug}`;
          return (
