...
const Header = styled.h2`
  ...
  color: #c22b70;
  text-transform: ${({ headerUpperCase }) =>
    headerUpperCase ? 'uppercase' : 'lowercase'};
...
const Warp = styled.div`
  ...
  box-shadow: ${({ sectionBorder }) =>
    sectionBorder ? '0 2px 0 rgba(0,0,0,.1)' : false};
  background-color: ${({ sectionBgGrey }) =>
    sectionBgGrey ? 'rgb(250, 251, 251);' : false};
  margin-bottom: 20px;
  ul {
    list-style: none;
    margin-bottom: 15px;
...
export default ({
  ...
  return (
    <Warp {...{ sectionBorder, sectionBgGrey }}>
      {header && (
        <Header {...{ headerUpperCase, headerSizeLg }}>{header}</Header>
      )}
      {children}
    </Warp>
  );
